...
def call_groq_llm(messages: list, temperature: float = None, json_mode: bool = False) -> str:
    if GROQ_API_KEY:
        return call_llm(messages, temperature, provider="groq", json_mode=json_mode)
    return call_llm(messages, temperature, json_mode=json_mode)


def call_routing_llm(messages: list, temperature: float = 0.0, json_mode: bool = False) -> str:
    return call_groq_llm(messages, temperature, json_mode=json_mode)
