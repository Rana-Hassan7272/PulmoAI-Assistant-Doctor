...
def patient_intake_agent(state: AgentState) -> AgentState:
    ...
    if not state.get("patient_data_confirmed"):
        ...
        # Validate and coerce extracted data via Pydantic schema
        ...
        if extracted.symptoms and not state.get("symptoms"):
            state["symptoms"] = extracted.symptoms
        if extracted.duration and not state.get("symptom_duration"):
