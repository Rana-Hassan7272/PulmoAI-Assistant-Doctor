...
def _symptom_based_tests(symptoms: str) -> List[str]:
    from .tools import recommend_tests
    return recommend_tests(symptoms)
...
def recommend_tests_llm(state: AgentState) -> List[str]:
    ...
    heuristic = _symptom_based_tests(symptoms)

    messages = [
        {
            "role": "system",
            "content": (
                "You are a pulmonology assistant. Pick the MINIMUM necessary tests "
                "from ONLY: xray, cbc, spirometry.\n"
                'Return JSON: {"tests": ["..."], "reason": "..."}\n\n'
                "CRITICAL: Do NOT recommend all three by default. Most visits need only 1 or 2 tests.\n\n"
                "Examples:\n"
                '- "dry cough, allergy" → {"tests": ["spirometry"]}\n'
                '- "cough, chest pain, infection" → {"tests": ["xray", "cbc"]}\n'
                '- "fever, productive cough" → {"tests": ["xray", "cbc"]}\n'
                '- "wheezing, shortness of breath" → {"tests": ["spirometry"]}\n'
                '- "severe pneumonia suspicion with fever and chest pain" → {"tests": ["xray", "cbc", "spirometry"]}\n\n'
                "Spirometry = lung function / wheeze / asthma / COPD — NOT for every cough.\n"
                "X-ray = chest pain, pneumonia, lung infection imaging.\n"
                "CBC = infection, fever, systemic illness.\n"
        ...
        },
        {
            ...
            "content": (
                ...
                f"Heuristic suggestion (you may use fewer): {heuristic}"
            ),
        },
    ]
    ...
    try:
        ...
        if tests:
            if len(tests) == 3 and len(heuristic) <= 2:
                tests = heuristic
    ...
    logger.info(f"Heuristic recommended tests: {heuristic}")
    return heuristic
...
def parse_approval_intent(state: AgentState) -> Dict[str, Any]:
    ...
    if re.search(
        r"\b(yes|approve|accepted?|proceed|agreed|finalize|finalise|finish|complete|okay?)\b",
        lower,
    ):
        return {"intent": "approve", "reason": "pattern"}
    if any(p in lower for p in ("looks good", "generate report", "go ahead")):
        return {"intent": "approve", "reason": "pattern"}
