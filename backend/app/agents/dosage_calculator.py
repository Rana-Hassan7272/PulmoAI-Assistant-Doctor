...
from .schemas import DosageOutput
import re
import logging
...
Provide the calculated dosage in the specified JSON format."""
    ...
    try:
        ...
        dose = DosageOutput.model_validate(raw)
        return dose.model_dump()
    except Exception as e:
        logger.warning(f"LLM dosage calculation failed: {e}")
        return DosageOutput().model_dump()
