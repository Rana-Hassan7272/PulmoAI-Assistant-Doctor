...
class BloodCountPredictor:
    ...
    def preprocess(self, data: Union[Dict, pd.DataFrame]) -> np.ndarray:
        ...
        # Step 1: IQR × 3 outlier capping on ALL features (matches training pipeline)
        ...
        # Step 2: RobustScaler on ALL features (matches training: scaler.fit_transform(X_res))
        ...
        if scaler_path.exists():
            try:
                ...
                X_array = fitted_scaler.transform(X_array).astype(np.float32)
            except Exception as e:
                import warnings
                warnings.warn(f"Could not load scaler.pkl: {e}. Predictions may be less accurate.")
